...
def print_top10_contributing_users():
    ...
    for doc in result:
        pprint.pprint(doc)
        user_count_dict[ doc['_id'] ] = doc['count']
        
    users_dataframe = pd.DataFrame(user_count_dict.items(), columns=['User', 'Count'])

    users_dataframe = pd.DataFrame.sort(users_dataframe, columns="Count", ascending=False)

    sns.set()
    sns.set(style="darkgrid")
    ax = sns.barplot(x ="User", y="Count", data=users_dataframe)
    for text in ax.get_xticklabels(): #rotate the labels on the x-axis
        text.set_rotation(90)
    plt.tight_layout()    
    plt.savefig("top10_contributing_users.png")
