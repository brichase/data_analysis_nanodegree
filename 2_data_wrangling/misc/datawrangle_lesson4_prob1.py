...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            # YOUR CODE HERE
            label = re.sub(r'\(.*?\)',"",line['rdf-schema#label']).strip()
            if label == "NULL":
                label = None
            ...
            description = line['rdf-schema#comment'].strip()
            if description == "NULL":
                description = None
            ...
            if synonym == "NULL":
                ...
                synonym = synonym.strip('{}').split('|')
                
            family = line['family_label'].strip()
            if family == "NULL":
                family = None
                
            class_label = line['class_label'].strip()
            if class_label == "NULL":
                class_label = None
                
            phylum = line['phylum_label'].strip()
            if phylum == "NULL":
                phylum = None
            
            order = line['order_label'].strip()
            if order == "NULL":
                order = None
                
            kingdom = line['kingdom_label'].strip()
            if kingdom == "NULL":
                kingdom = None
            
            genus = line['genus_label'].strip()
            if genus =="NULL":
                genus = None
