    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            if line['binomialAuthority_label'] == "NULL":
                continue
            rSchema= re.sub(r'\(.*?\)',"",line['rdf-schema#label']).strip()
            data[rSchema] = line['binomialAuthority_label']
...
def update_db(data, db):
    ...
    for key in process_data:
        print data[key]
        arachnid = db.arachnid.find_one({"label": key})
        arachnid["classification"]["binomialAuthority"] = data[key]
        db.arachnid.save(arachnid)
    pass
